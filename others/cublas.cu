...
int main() {
    ...
    std::vector<Block> hDev(numBlocks);
    for(int i = 0; i < numBlocks; ++i) {
        auto &hb = blocks[i];
        auto &db = hDev[i];
        db.type       = hb.type;
        db.row_offset = hb.row_offset;
        db.col_offset = hb.col_offset;
        db.m          = hb.m;
        db.n          = hb.n;
        if(hb.type == DENSE) {
            size_t bytes = hb.m * hb.n * sizeof(float);
            CUDA_CHECK(cudaMalloc(&db.d_dense, bytes));
            CUDA_CHECK(cudaMemcpy(db.d_dense, hb.denseData, bytes,
                                cudaMemcpyHostToDevice));
            db.d_U = db.d_V = nullptr;
        } else {
            size_t Ub = hb.m * globalKRank * sizeof(float);
            size_t Vb = hb.n * globalKRank * sizeof(float);
            CUDA_CHECK(cudaMalloc(&db.d_U, Ub));
            CUDA_CHECK(cudaMalloc(&db.d_V, Vb));
            CUDA_CHECK(cudaMemcpy(db.d_U, hb.U, Ub,
                                cudaMemcpyHostToDevice));
            CUDA_CHECK(cudaMemcpy(db.d_V, hb.V, Vb,
                                cudaMemcpyHostToDevice));
            db.d_dense = nullptr;
        }
    }
    // Copy the metadata array itself to device
    Block *dBlocks = nullptr;
    CUDA_CHECK(cudaMalloc(&dBlocks, numBlocks * sizeof(Block)));
    CUDA_CHECK(cudaMemcpy(dBlocks, hDev.data(), numBlocks * sizeof(Block),
                        cudaMemcpyHostToDevice));

    // Assuming N is your full matrix dimension:
    float *d_x, *d_y;
    CUDA_CHECK(cudaMalloc(&d_x, n * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_y, n * sizeof(float)));

    CUDA_CHECK(cudaMemcpy(d_x, x_h.data(),
                        n * sizeof(float),
                        cudaMemcpyHostToDevice));
    // Zero-out y, we'll accumulate into it
    CUDA_CHECK(cudaMemset(d_y, 0, n * sizeof(float)));

    cublasHandle_t handle1;
    cublasCreate(&handle1);

    // We'll need one temporary buffer α⃗ of length globalKRank:
    float *d_alpha;
    CUDA_CHECK(cudaMalloc(&d_alpha, globalKRank * sizeof(float)));
    ...
    for(int b = 0; b < numBlocks; ++b) {
        // Copy one DevBlock back to host (or keep a host copy)
        Block db;
        CUDA_CHECK(cudaMemcpy(&db, dBlocks + b, sizeof(Block),
                            cudaMemcpyDeviceToHost));

        if(db.type == DENSE) {
            // y[row_offset : row_offset+ m] += A * x[col_offset : col_offset+n]
            cublasSgemv(handle,
                        CUBLAS_OP_N,
                        db.m,           // rows of A
                        db.n,           // cols of A
                        ...
                        d_x + db.col_offset,
                        1,
                        &beta_one,      // accumulate into y
                        d_y + db.row_offset,
                        1);
        }
        else {
            // 1) α⃗ = Vᵀ (n×k)  · x[col_offset : n]
            cublasSgemv(handle,
                        CUBLAS_OP_T,
                        db.n,                // rows of V
                        globalKRank,         // cols = k
                        &alpha,
                        db.d_V,
                        db.n,                // leading dim of V
                        d_x + db.col_offset,
                        1,
                        &beta_zero,          // β = 0 → overwrite d_alpha
                        d_alpha,
                        1);

            // 2) y[row_offset : m] += U (m×k) · α⃗
            cublasSgemv(handle,
                        CUBLAS_OP_N,
                        db.m,                // rows of U
                        globalKRank,         // cols = k
