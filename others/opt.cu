...
constexpr int TILE_SIZE = 128;             // tile dimension for dense tasks
constexpr int MAX_RANK  = 32;              // maximum low-rank k
int globalKRank = 0;
...
enum TaskType { DENSE_TILE=0, LR_STEP1=1, LR_STEP2=2 };
struct Task {
    TaskType  t;           // which micro-operation
    int       row, col;    // tile origin in global coords
    int       m,n,k;       // dims (k=rank for LR tasks)
    float*    A;           // data ptr: tile or U/V
};

//------------------------------------------------------------------------------
// 2.  CPU helpers: tiling & grouping
//------------------------------------------------------------------------------
__host__
void tile_dense_block(const Block& B, std::vector<Task>& tasks) {
    int mt = (B.m + TILE_SIZE-1)/TILE_SIZE,
        nt = (B.n + TILE_SIZE-1)/TILE_SIZE;
    for(int i=0;i<mt;i++) for(int j=0;j<nt;j++){
        int m0 = TILE_SIZE, n0 = TILE_SIZE;
        if((i+1)*TILE_SIZE > B.m) m0 = B.m - i*TILE_SIZE;
        if((j+1)*TILE_SIZE > B.n) n0 = B.n - j*TILE_SIZE;
        Task t;
        t.t    = DENSE_TILE;
        t.row  = B.row_offset + i*TILE_SIZE;
        t.col  = B.col_offset + j*TILE_SIZE;
        t.m    = m0; t.n = n0; t.k = 0;
        t.A    = B.denseData + (i*TILE_SIZE)*B.n + (j*TILE_SIZE);
        tasks.push_back(t);
    }
}

__host__
void tile_lowrank_block(const Block& B, std::vector<Task>& tasks) {
    // 1) V^T x  → produces a k-vector α
    Task t1; t1.t = LR_STEP1;
    t1.row = B.row_offset; t1.col = B.col_offset;
    t1.m   = B.n;     t1.k = globalKRank; t1.n = 0;
    t1.A   = B.V;
    tasks.push_back(t1);
    // 2) U α  → scatter into output rows
    Task t2; t2.t = LR_STEP2;
    t2.row = B.row_offset; t2.col = B.col_offset;
    t2.m   = B.m;     t2.k = globalKRank; t2.n = 0;
    t2.A   = B.U;
    tasks.push_back(t2);
}

__host__
bool group_by_shape(const Task& a, const Task& b) {
    return a.t==b.t && a.m==b.m && a.n==b.n && a.k==b.k;
}

//------------------------------------------------------------------------------
// 3.  Device: persistent kernel + micro‑kernels
//------------------------------------------------------------------------------
__device__ int gTaskCounter = 0;
...
__device__
void process_task(const Task& tk, const float* __restrict__ x, float* __restrict__ y) {
    if(tk.t == DENSE_TILE) {
        // one warp per TILE_SIZE‑row chunk
        int lane = threadIdx.x & 31, wid = threadIdx.x >> 5;
        int rows_per_warp = (tk.m + (blockDim.x/32)-1)/(blockDim.x/32);
        int r0 = wid*rows_per_warp;
        for(int r=r0; r<tk.m && r<r0+rows_per_warp; ++r){
            // load A[r,*] into registers
            float sum = 0.0f;
            ...
            if(lane==0){
                atomicAdd(&y[tk.row + r], sum);
            }
        }
    }
    else if(tk.t == LR_STEP1) {
        // compute α = V^T x
        int lane = threadIdx.x;
        float alpha = 0.0f;
        for(int i=lane; i<tk.k; i+=blockDim.x){
            alpha += tk.A[i + 0*tk.k] * x[tk.col + 0];  // & subsequent columns
            // note: for brevity we assume packed contiguously
        }
        alpha = warp_reduce_sum(alpha);
        if(lane==0){
            // write into shared buffer (not shown) or global tmp array
        }
    }
    else if(tk.t == LR_STEP2) {
        // scatter U α
        int lane = threadIdx.x;
        for(int r=lane; r<tk.m; r+=blockDim.x){
            float val = 0.0f;
            for(int j=0; j<tk.k; ++j){
                // read α[j] from shared/glob tmp
                float alpha_j = 0.0f; 
                val += tk.A[r*tk.k + j] * alpha_j;
            }
            if(val != 0.0f)
                atomicAdd(&y[tk.row + r], val);
        }
    }
}

__global__
void hodlr_matvec(const Task* tasks, int numTasks,
                  const float* x, float* y)
{
    int id = atomicAdd(&gTaskCounter, 1);
    while(id < numTasks){
        process_task(tasks[id], x, y);
        id = atomicAdd(&gTaskCounter, 1);
    }
}
