...
constexpr int ROW_SPLIT_FACTOR = 4;
...
void compressHODLR(const std::vector<float>& A, int N,
    ...
    {
        auto sub = getSubMatrix(A, N, row_offset, col_offset+mid, mid, s-mid);
        std::vector<float> Ufull, Vfull;
        dummySVD(sub, mid, s-mid, globalKRank, Ufull, Vfull);
        int rows = mid, cols = s-mid;

        // Split into ROW_SPLIT_FACTOR chunks
        int chunkRows = (rows + ROW_SPLIT_FACTOR - 1) / ROW_SPLIT_FACTOR;
        for(int chunk=0; chunk<ROW_SPLIT_FACTOR; ++chunk){
            int r0 = chunk*chunkRows;
            int rcount = std::min(chunkRows, rows - r0);
            if(rcount <= 0) break;

            Block B{LOW_RANK,
                    /* row_offsetset */ row_offset + r0,
                    /* col_offset */ col_offset + mid,
                    /* m */ rcount,
                    /* n */ cols,
                    /* denseData */ nullptr,
                    /* U */ nullptr,
            ...
            // Allocate & copy U slice
            B.U = new float[rcount * globalKRank];
            std::memcpy(B.U,
                        Ufull.data() + r0 * globalKRank,
                        rcount * globalKRank * sizeof(float));

            // Allocate & memcpy full V
            B.V = new float[cols * globalKRank];
            std::memcpy(B.V,
                        Vfull.data(),
                        cols * globalKRank * sizeof(float));
    ...
    {
        auto sub = getSubMatrix(A, N, row_offset+mid, col_offset, s-mid, mid);
        std::vector<float> Ufull, Vfull;
        dummySVD(sub, s-mid, mid, globalKRank, Ufull, Vfull);
        ...
        for(int chunk=0; chunk<ROW_SPLIT_FACTOR; ++chunk){
            ...
            Block B{LOW_RANK,
                    /* row_offsetset */ row_offset + mid + r0,
...
__global__ void hodlr_mvm_kernel(const Block* blocks, int numBlocks,
    ...
    // Shared buffer for reduction / low-rank α
    extern __shared__ float sdata[];

    if (block.type == DENSE) {
        // 1. Grid-stride partial sums
        float localSum = 0.0f;
        for (int j = threadIdx.x; j < block.n; j += blockDim.x) {
            localSum += block.denseData[localRow*block.n + j] * x[block.col_offset + j];
        }
        ...
        if (lid == 0)
            atomicAdd(&y[block.row_offset + localRow], sdata[0]);
    } 
    else {
        // Phase 1: V^T x, one warp per r
        int warpId = (threadIdx.x >> 5);
        int lane   = (threadIdx.x & 31);
        for (int r = 0; r < k; ++r) {
            float dot = 0.0f;
            for (int j = lane; j < block.n; j += 32) {
                dot += block.V[j*k + r] * x[block.col_offset + j];
            }
            dot = warp_reduce_sum(dot);
            if (lane == 0) {
                sdata[warpId*k + r] = dot;
            }
        }
        __syncthreads();
        // Phase 2: U * α
        float localRes = 0.0f;
        for (int r = 0; r < k; ++r) {
            localRes += block.U[localRow*k + r] * sdata[warpId*k + r];
        }
        if (threadIdx.x == 0)
            atomicAdd(&y[block.row_offset + localRow], localRes);
    }
}
